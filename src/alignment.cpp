...
run (int argc, char **argv)
{
  ...
  {
    scene_clouds.push_back (pcl::PointCloud<pcl::PointNormal>::Ptr (new pcl::PointCloud<pcl::PointNormal>));
    BOOST_LOG_TRIVIAL (info) << boost::format ("Loading scene point cloud: %s") % scene_file.c_str ();
  ...
  for (int i = 0; i < model_files.values.size (); i++)
  {
    ...
    model_clouds.push_back (pcl::PointCloud<pcl::PointNormal>::Ptr (new pcl::PointCloud<pcl::PointNormal>));

    BOOST_LOG_TRIVIAL (info) << boost::format ("Loading model point cloud: %s") % model_file.c_str ();
    ...
    float3 model_diam = {maxPt.x - minPt.x, maxPt.y - minPt.y, maxPt.z - minPt.z};
    model_d_dists.push_back (tau_d[i] * max (model_diam));
    BOOST_LOG_TRIVIAL (debug) << boost::format ("model_diam, d_dist: (%f, %f, %f), %f") % model_diam.x % model_diam.y
                                   % model_diam.z % model_d_dists.back ();
  }
  ...
  {
    BOOST_LOG_TRIVIAL (info) << boost::format ("Scene size before filtering: %u (%u x %u)") % scene->size ()
                                  % scene->width % scene->height;
    pcl::PointCloud<pcl::PointNormal>::Ptr new_scene = voxelGridDownsample<pcl::PointNormal> (scene, scene_leaf_size);
    scene = new_scene;
    BOOST_LOG_TRIVIAL (info) << boost::format ("Scene size after filtering: %u (%u x %u)") % scene->size ()
                                  % scene->width % scene->height;
  }
  ...
  for (int i = 0; i < model_clouds.size (); i++)
  {
    ...
    BOOST_LOG_TRIVIAL (info) << boost::format ("Model size before filtering: %u (%u x %u)") % model->size ()
                                  % model->width % model->height;
    pcl::PointCloud<pcl::PointNormal>::Ptr new_model = voxelGridDownsample<pcl::PointNormal> (model, model_d_dists[i]);
    BOOST_LOG_TRIVIAL (info) << boost::format ("Model size after filtering: %u (%u x %u)") % new_model->size ()
                                  % new_model->width % new_model->height;
    model_clouds[i] = new_model;
  }

  std::vector<std::vector<Eigen::Matrix4f>> results = ppf_registration (
    scene_clouds, model_clouds, model_d_dists, vm["ref_point_df"].as<unsigned int> (),
    vm["vote_count_threshold"].as<float> (), vm["cpu_clustering"].as<bool> (), vm["use_l1_norm"].as<bool> (),
    vm["use_averaged_clusters"].as<bool> (), vm["dev"].as<int> (), NULL);
