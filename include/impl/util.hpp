...
void
histogram (const Vector1 &input, // assumed to be already sorted
           Vector2 &histogram_values, Vector3 &histogram_counts)
{
  ...
  IndexType num_bins =
    thrust::inner_product (data.begin (), data.end () - 1, data.begin () + 1, IndexType (1), thrust::plus<IndexType> (), thrust::not_equal_to<ValueType> ());
...
template <typename T>
void
write_array (const char *filename, T *data, int n)
{
  FILE *fp;
...
template <typename T>
void
write_device_array (const char *filename, T *data, int n)
{
  T *host_array = new T[n];
  if (cudaMemcpy (host_array, data, n * sizeof (T), cudaMemcpyDeviceToHost) != cudaSuccess)
  {
...
template <typename T>
void
write_device_vector (const char *filename, thrust::device_vector<T> *data)
{
...
std::istream &
operator>> (std::istream &s, Eigen::MatrixBase<Derived> &m)
{
